...
var HuggingfaceCli = struct {
	...
	AWS_ECR_PUBLIC_IMAGE_NAME:       "huggingface-cli",
	AWS_ECR_PUBLIC_IMAGE_TAG:        "latest",
}

func TestContainersGoPullHuggingfaceCli(t *testing.T) {
	ctx := context.Background()
	for attempt := 0; attempt < 3; attempt++ {
		container, e := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
			ContainerRequest: testcontainers.ContainerRequest{
				Image: HuggingfaceCli.AWS_ECR_PUBLIC_URI + "/" + HuggingfaceCli.AWS_ECR_PUBLIC_REPOSITORY_GROUP + "/" + HuggingfaceCli.AWS_ECR_PUBLIC_IMAGE_NAME + ":" + HuggingfaceCli.AWS_ECR_PUBLIC_IMAGE_TAG,
			},
		})
		require.NoError(t, e)
...
func TestContainersGoExecHuggingfaceCli(t *testing.T) {
	ctx := context.Background()
	container, e := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			Image: HuggingfaceCli.AWS_ECR_PUBLIC_URI + "/" + HuggingfaceCli.AWS_ECR_PUBLIC_REPOSITORY_GROUP + "/" + HuggingfaceCli.AWS_ECR_PUBLIC_IMAGE_NAME + ":" + HuggingfaceCli.AWS_ECR_PUBLIC_IMAGE_TAG,
